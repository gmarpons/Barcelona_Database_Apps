...
La informació que apareix a continuació a sigut recollida mitjançant enquestes fetes a través d'internet ([http://goo.gl/OfqwdC](http://goo.gl/OfqwdC)). Tot i que l'enquesta segueix en funcionamenta, les dades que aquí es presentan van ser descarregades el 28 de gener del 2015. L'objectiu d'aquesta descàrrega inicial és la de poder aportar informacions que puguin ser d'utilitat pel diagnóstic dels barris. És molt important ser conscients de que **la informació que presentem no té cap significació estadística** (i menys a nivell de barris) **que permeti fer cap tipus d'inferència**. 

En pràcticament tots els casos, la mostra aconseguida no és prou gran. A més, aquesta mostra no és mai, com veurem a continuació, aleatòria. És per això que demanem NO pendre les informacions recollides en aquesta secció com a dades representatives del conjunt de la població del barri o districte, sinó més aviat com a opinions puntuals a títol personal. Per descomptat, aquests apunts sobre el conjunt de la mostra no li treuen gens d'importància a cadascuna de les valuoses opinions recollides i que també expliquen part de la realitat del barri o districte.
...
## Enquestes al `r analyzing` de '`r toupper(name_analyzed)`' (mostra)

La mostra consta de `r round(results["enq1_respostes",1],0)` enquestes realitzades des de  `r round(results["enq1_ips_unicas",1],0)` IPs diferents. Com es pot comprovar a continuació, el perfil de les persones enquestades no és representatiu del conjunt de la població, mostrant alguns biaixos importants:

.

```{r INTERVIEW_PLOTS, echo=F, results="hide"}

#par(oma=c(3,3,5,3))
#par(mar=c(5,4,5,2) + 0.1) 
par(mfrow=c(2,2))

barx <- barplot(c(results["enq1_dones",1],results["enq1_homes",1],results["enq1_altre_genere",1]), names.arg=c("Dona","Home.","Altres"), main="Identificació pròpia del gènere", sub="de l'Ajuntament al barri/districte)", xlab="genere (punts vermells per les dades", ylab="% de persones", ylim=c(0,55), cex.names=0.8)
points (barx,c(results["enq1_dones",2],results["enq1_homes",2],-3), col="darkred", bg="darkred", pch=8, lwd=2)

barx <- barplot(c(results["enq1_edats_.15",1],results["enq1_edats_15_24",1],results["enq1_edats_25_39",1],results["enq1_edats_40_64",1],results["enq1_edats_.64",1],results["enq1_edats_NA",1]), names.arg=c("<15","15-24","25-39","40-64",">64","NS/NC"), main="Edat de la mostra (+ població)", sub="l'edat de tot el barri/districte)", xlab="edats (punts vermells per indicar", ylab="% de persones", cex.names=0.8)
points (barx,c(results["enq1_edats_.15",2],results["enq1_edats_15_24",2],results["enq1_edats_25_39",2],results["enq1_edats_40_64",2],results["enq1_edats_.64",2],-3), col="darkred", bg="darkred", pch=8, lwd=2)

barx <- barplot(c(results["enq1_edu_prim",1],results["enq1_edu_secu",1],results["enq1_edu_univ",1],results["enq1_edu_NA",1]), names.arg=c("Prim.","Secun.","Univ.","NS/NC"), main="Estudis cursats per la mostra (+ població)", sub="estudis de tot el barri/districte)", xlab="estudis cursats (punts vermells pels", ylab="% de persones", cex.names=0.8)
points (barx,c(results["enq1_edu_prim",2],results["enq1_edu_secu",2],results["enq1_edu_univ",2],-3), col="darkred", bg="darkred", pch=8, lwd=2)

barx <- barplot(c(results["enq1_associ_si",1],results["enq1_associ_no",1],results["enq1_associ_NA",1]), names.arg=c("Sí","No","NS/NC"), main="Participació en teixit associatiu", sub="la resta de la població del barri/districte)", xlab="participació (sense comparacions possibles amb", ylab="% de persones", cex.names=0.8)

```


## Opinions sobre la situació al barri
...
```{r HOUSING_OPINION_PLOTS, echo=F, results="hide"}

par(mfrow=c(1,2))

barx <- barplot(c(results["enq1_vivenda_problem_no",1],results["enq1_vivenda_problem_alguns",1],results["enq1_vivenda_problem_greus",1],results["enq1_vivenda_problem_NA",1]), names.arg=c("No\nn'hi ha","Alguns\nproblemes","Problemes\ngreus","NS/NC"), main="Hi ha problemes d'accés a l'habitatge?", sub="per anàlisi conjunt de tots els barris de la ciutat)", xlab="creuen que hi ha problemes (punts verds", ylab="% de persones", cex.names=0.8)
...
barx <- barplot(c(results["enq1_vivenda_prob_preu",1],results["enq1_vivenda_prob_oferta",1],results["enq1_vivenda_prob_oferta_pub",1],results["enq1_vivenda_prob_credit",1],results["enq1_vivenda_prob_condicions",1],results["enq1_vivenda_prob_desnona",1],results["enq1_vivenda_prob_buits",1]), main="Quin és el principal problema?", sub="és anàlisi conjunt de tots els barris de la ciutat)", xlab="índex de preocupació (punts verds", cex.names=0.8, horiz=T, las=1, xlim=c(0,30))
...
n_results <- n_results + 1
results[n_results,2] <-  NA
rownames(results)[n_results] <- "enq1_vivenda_prob_altres"
for (i in 1:nrow(barris)) {
  if (barris[i,"enq1_vivenda_prob_altres"]!="" & !is.na(barris[i,"enq1_vivenda_prob_altres"])) {
    results[n_results,1] <- paste(results[n_results,1],barris[i,"enq1_vivenda_prob_altres"], sep=" / ")
  }
}
```

Altres problemes del barri relacionats amb la vivenda i enumerats durant les enquestes (sense filtratge previ): *`r results["enq1_vivenda_prob_altres",1]`*.
